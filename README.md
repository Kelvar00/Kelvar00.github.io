...
以下说明面向当前仓库的“只保留 About 页面”配置。

### 1) 你最常改的文件

- 个人主页正文：`_pages/about.md`
  - 改自我介绍、研究方向、论文列表。
  - `announcements.enabled` 和 `latest_posts.enabled` 控制是否在 About 显示新闻和博客。
- 站点基本信息：`_config.yml`
  - `first_name` / `middle_name` / `last_name`：首页标题姓名。
  - `description`：站点描述。
  - `url` / `baseurl`：部署地址（GitHub Pages 必改）。
- 社交信息：`_data/socials.yml`
  - 例如邮箱、ORCID。

### 2) announcements 和 latest_posts 的内容放哪里

- announcements 来源：`_news/` 下的条目（Markdown 文件）。
- latest_posts 来源：`_posts/` 下的博客文章。

当前仓库里 `_news/` 只保留了隐藏 sample。需要启用时：

1. 复制 `_news/announcement_1.md` 作为新文件（例如 `announcement_2026_01.md`）。
2. 把 frontmatter 里的 `published: false` 改为 `published: true`。
3. 在 `_pages/about.md` 里把 `announcements.enabled: true`。

### 3) 头像大小和位置怎么调

头像配置在 `_pages/about.md` 的 `profile` 字段：

- 位置：
  - `align: right`（右侧）
  - `align: left`（左侧）
- 形状：
  - `image_circular: true`（圆形）
  - `image_circular: false`（圆角矩形）
- 图片文件：
  - `image: head.jpg`（图片放在 `assets/img/`）

大小有两种调法：

1. **直接换更大/更小分辨率的原图（推荐）**：最稳，不影响其他布局。
2. **改 About 布局中的宽度比例**：编辑 `_layouts/about.liquid`，其中 `sizes` 默认使用 `site.max_width * 0.3`，把 `0.3` 调大（头像更大）或调小（头像更小）。

### 4) 数据目录现在的策略

- `_news/`、`_books/`、`_bibliography/`、`_data/` 已清为最小样例。
- 样例都带注释，默认不会显示，可按注释逐步恢复。
